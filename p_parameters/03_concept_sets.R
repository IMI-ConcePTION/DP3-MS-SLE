...
File_codelist_MS_SLE <- paste0(thisdir,"/p_parameters/archive_parameters/MS-SLE codelist.xlsx")
OUT_codelist <- as.data.table(readxl::read_excel(File_codelist_MS_SLE, sheet = "Diagnosis Codes",
                                                 .name_repair = ~ vctrs::vec_as_names(..., repair = "universal",
                                                                                      quiet = T)))

setnames(OUT_codelist, "Disease", "Conceptname")

CONCEPT_list <- unique(OUT_codelist$Conceptname)

OUT_codelist <- lapply(split(OUT_codelist, by = "Conceptname", keep.by = F),
            split, by = "Coding.system", keep.by = F)
concept_set_codes_our_study <- lapply(OUT_codelist, sapply, unlist, use.names = F, simplify = F)
...
DRUG_codelist <- as.data.table(readxl::read_excel(File_codelist_MS_SLE, sheet = "Medications",
                                                  .name_repair = ~ vctrs::vec_as_names(..., repair = "universal",
                                                                                       quiet = T)))
DRUG_codelist[, Coding.system := "ATC"][, Disease := paste(Disease, Tag, sep = "_")]
DRUG_codelist <- DRUG_codelist[, c("Tag", "Medication") := NULL]

DRUG_codelist <- lapply(split(DRUG_codelist, by = "Disease", keep.by = F),
                       split, by = "Coding.system", keep.by = F)
DRUG_codelist_list <- lapply(DRUG_codelist, sapply, unlist, use.names = F, simplify = F)
rm(DRUG_codelist)
...
rm(File_codelist_MS_SLE, concept)
