...
# Program:      formatdata2.R 
# Author:       adopted and slightly changed from function 'formatdata' (library SCCS version of sept 2021) by Svetlana Belitser
#
#  covariates can now be character, factor or numeric. You do not need anymore to use 'factor': factor(cov)!!!
#
#  ( comments Sv added where the code changed )

formatdata2 <- function (indiv, astart, aend, aevent, adrug, aedrug, expogrp = list(), 
                        ...
                        dob = NULL, cov = cbind(), dataformat = "stack", data) 
{
  ...
  if (is.null(season)) {
    chopdat <- data.frame(cbind(indivL = ind[interval != 0], 
                                event = event[interval != 0], 
                                eventday = eventday[interval != 0], 
                                lower = (cutp[interval != 0] - interval[interval != 0]) + 1, 
                                upper = cutp[interval != 0], interval = interval[interval != 0], 
                                age = agegr[interval != 0], exgr1[interval != 0, ]))
    
    chopdat <- chopdat[, -ncol(chopdat)]
    chopdat <- cbind.data.frame(chopdat, data1[ rep(1:nrow(data1), times = data.frame(table(chopdat$indivL))$Freq) , ] )   #Sv
    #Sv chopdat1 <- data.frame(cbind(chopdat, apply(data1, 2, 
    #Sv                                            function(x) rep(x, times = data.frame((table(chopdat$indivL)))$Freq))))
                                                                                                       ...
                                                                                                       0], lower = (cutp[interval != 0] - interval[interval != 
                                                                                                                                                     0]) + 1, upper = cutp[interval != 0], interval = interval[interval != 
                                                                                                                                                                                                                 0], season = season[interval != 0], age = agegr[interval != 
                                                                                                                                                                                                                                                                   0], exgr1[interval != 0, ]))
    chopdat <- chopdat[, -ncol(chopdat)]
    chopdat <- cbind.data.frame(chopdat, data1[ rep(1:nrow(data1), times = data.frame(table(chopdat$indivL))$Freq) , -ncol(data1) ] )  #Sv
    #Sv chopdat <- data.frame(cbind(chopdat, apply(data1[, -ncol(data1)], 
    #Sv                                            2, function(x) rep(x, times = data.frame((table(chopdat$indivL)))$Freq))))
  }
